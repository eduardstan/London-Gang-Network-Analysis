...
# Introduction
We work on the London-based inner-city street gang from $2005$ to $2009$, concerning black people, operating from a social housing estate. Data is available on [UCINET Software website](https://sites.google.com/site/ucinetsoftware/datasets/covert-networks/londongang "London Gang") and comes from anonymised police arrest and conviction data for **all confirmed** members of the gang.

The network is **undirected** and there has $54$ nodes representing the monitored people having the following attributes:

* `Person`, representing the person's **id**,
...
* `Residence`, representing the fact that the person is **resident**,
...
* `Prison`, representing the fact that such person is **arrested**,
* `Music`, representing the fact that such person **listens to music**,
* `Ranking`, representing a **ranking** of such person.

The ties of such network are given as an weighted adjacency matrix, i.e. a $54 \times 54$ (weighted) matrix, where the weights are: 

* $1$, representing the fact that they have **hanged-out**, 
* $2$, representing the fact that they have **co-offended together**, 
* $3$, representing the facts that they have **co-offended together** and that they have **committed serious crimes**, 
* $4$, representing the facts that they have **co-offended together**, **committed serious crimes** and that there is a **kin** relationship (i.e. there is a family relationship in-between nodes).


## Installation
Before we begin our (tidy approach) analysis, first we need to install the following packages:
```{r, eval=F}
...
# Data import
We read the CSV files and store such information, and print some preliminary information about such structures as:
```{r}
relations <- as_tibble(read.table("london/gang.csv", header=TRUE, sep=",")) %>%
  # select all columns, but not the first one
  select(X1:X54)
people <- read_csv("london/attr.csv", col_names=TRUE) %>%
...
Now, we are ready to build our graph using the `igraph` package and to (preliminarly) plot such object as:
```{r}
...
ggplot(nodes, aes(x=age, stat(density))) +  # distribution for age
  geom_histogram(binwidth=1.2, alpha=2/5) +
...
ggplot(nodes, aes(x=arrests, stat(density))) +  # distribution for arrests
  geom_histogram(binwidth=2, alpha=2/5) +
  geom_density(color="red", size=1)
...
ggplot(nodes, aes(x=birthplace, y=arrests)) + # arrests as a function of birthplace
  geom_boxplot(aes(color=birthplace))
...
Note that, although we have a small dataset, we can say that people that come from West Africa (i.e. `birthplace` is equal to $1$) tend to be more criminals, witnessed by the fact that the arrests and convinctions are (in general) higher. Moreover, people that do not have a `residence` tend to be more criminal, and the reasoning is similar.
...
ggplot(nodes, aes(x=age, y=arrests)) + # arrests as a function of age
  geom_point(aes(color=birthplace), size=3) +
  geom_smooth()
...
Note that, we have an expected behaviour, that is, people tend to be less criminal until a certain age, in this case $\approx 19-20$ years old, then they tend to be more criminal. Perhaps this is due the fact that until a fragile age, the parents of such individuals assume the full-responsability of their own childs, but then such individuals are grown-up and tend to act as criminals.
...
We can use the `ggraph` package to plot a beautiful graph where we color the edges based on their weights (`weight`) and resize the nodes based on their PageRank (`pr`) centrality as:
```{r}
ggraph(g, layout = "with_kk") + 
  geom_edge_link(aes(color = weight)) + # edge link geom, mapping color to weight
...
Weak ties are very important for several reasons. **todo motivations**. We can compute such ties in a straightforward way as:
```{r}
ties %>%
...
It seems that $\approx 58\%$ of the people in the network have hanged-out with each other, that is, perhaps **hanging out with the "wrong" people may lead people to commit (serious) crimes**. Therefore, reasoning by complementation, there are $\approx 42\%$ of the people that have done some crimes together.
...
To answer this question, we use the **Pearson correlation coefficient** (other methods can be used, such as **cosine similarity**) to detect similarities. In order to compute such metric, we extrapolate the adjacency matrix from our graph `g` and compute the correlation matrix as:
```{r}
...
ggplot(as.tibble(V), aes(x=value)) +
  geom_histogram(aes(alpha=2/5), bins=30, show.legend=FALSE) +
  geom_freqpoly(aes(color="red"), size=1, bins=30, show.legend=FALSE)
```
...
prop_sim <- cnt/length(V) # proportion of similar nodes
print(prop_sim)           # print such proportion
print(1-prop_sim)         # print the 1-complementary proportion (i.e. dissimilar proportion)
```

Not surprisingly, there is a **lot of similarity** between nodes, and this is an expected behavior. Moreover, let's try to give an answer to the following question:
...
# Power analysis
We are interested in comparing the centrality of people against their (PageRank) power. In particular, we would like to identify people that are central and powerful. In order to perform such analysis, we define the following functions (powered by Massimo Franceschet):
```{r}
...
```{r}
mc <- quantile(nodes$pr, 0.75)    # 0.75 pagerank centrality quantile
mp <- quantile(nodes$power, 0.75) # 0.75 power measure quantile
plot(nodes$power, nodes$pr, xlab="power", ylab="centrality", pch=NA, bty="l")
text(nodes$power, nodes$pr, labels=nodes$person, adj=c(0.5,0.5), cex=1, col = "lightblue")
abline(h=mc, lty=3)               # horizontal line
abline(v=mp, lty=3)               # vertical line
...
```{r}
simil_power_central_nodes <- power_central_nodes %>%
  inner_join(simil_nodes, c("person" = "name"))

simil_power_central_nodes
```
...
Note that, given the graph and the `simil_power_central_nodes` structure, the person $3$ is isolated from the others, therefore we can exclude such person from the analysis. Therefore, to answer our question, we can do the following:
```{r}
# remove the node 3
simil_power_central_nodes <- simil_power_central_nodes %>%
  filter(person != 3)

simil_power_central_nodes # print most similar, powerful and central nodes (without 3)
...
# get the original weight (until now we have worked on the filtered similar pairs)
filtered_pairs %>% 
...
As you can see, not surprisingly, people that are similar, powerful and central (w.r.t the measures computed before) **stick to each other as family**, given in terms as the weight on the relation between such nodes (i.e. $4$ in this case); this answers our question, that is, we have a $100\%$ proportion of such subjects.

# Model
Suppose now that we would like to make predictions on individuals. For instance, we could try to classify people based on the fact that they have been arrested or not (i.e. `prison=1` or `prison=0`). Therefore, we can formalize a **classification problem**. Briefly, the classification setting lies within the statistical learning framework, where for each observation (the people, in our case) has a label (i.e. the fact that they have been arrested or not, in our aforementioned example). 
...
We then moved on with the centrality analysis of the network and have pointed out some (preliminary) interesting facts. For instance, the network is very well tied, that is, people have hanged-out together and more than $40 \%$ have commited crimes together.

In our opinion, the most interesting part of the presented analysis is relative to the part of the connection patterns between nodes. The criminality analysis was driven by the birthplace, and, perhaps not surprisingly, very important empirical proofs have outcomed. For instance, very connected people tend to act as criminals with their own people, that is, with those that came from their own birthplace. Moreover, strongly related people that are powerful and central also tend to commit crimes along with their families, and this is very natural if we think, for example, at the Italian Mafia. 